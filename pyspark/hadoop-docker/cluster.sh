...
function startServices {
  docker start nodemaster slave1
  sleep 5
  echo ">> Starting hdfs ..."
  ...
  docker exec -u hadoop -d slave1 start-slave.sh -c 1 -m 2G nodemaster:7077
  #docker exec -u hadoop -d slave2 start-slave.sh nodemaster:7077
  sleep 5
...
if [[ $1 = "install" ]]; then
  ...
  docker run -d --net hadoopnet --ip 172.20.1.1 -p 8088:8088 --hostname nodemaster --add-host slave1:172.20.1.2  --add-host archlinux:192.168.29.121 --name nodemaster -it hadoop_cluster:spark
  echo "******"
  docker run -d --net hadoopnet --ip 172.20.1.2 --hostname slave1 --add-host nodemaster:172.20.1.1 --add-host archlinux:192.168.29.121 --name slave1 -it hadoop_cluster:spark
  

  #docker run -d --net hadoopnet --ip 172.20.1.1 -p 8088:8088 --hostname nodemaster --add-host slave1:172.20.1.2 --add-host slave2:172.20.1.3 --name nodemaster -it sciencepal/hadoop_cluster:hive
...
function stopServices {
  ...
  docker exec -u hadoop -d slave1 stop-slave.sh
  ...
  #docker stop nodemaster slave1 slave2 edge hue nifi zeppelin psqlhms
  docker stop nodemaster slave1
}
...
if [[ $1 = "uninstall" ]]; then
  ...
  docker rmi hadoop_cluster:hadoop hadoop_cluster:spark -f
